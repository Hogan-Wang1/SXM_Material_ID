import os
import argparse
import tkinter as tk
from tkinter import filedialog
from pathlib import Path
import csv
from datetime import datetime
from dotenv import load_dotenv

# 加载 .env 文件中的环境变量
load_dotenv()

# ==========================================
# 导入项目模块
# ==========================================
try:
    # 1. 负责读取 .sxm 文件
    from modules.reader import read_sxm  
    
    # 2. 负责预处理、频域分析与晶格提取
    from modules.analyzer import STMAnalyzer 
    
    # 3. 负责基于对称性和几何特征的比对
    from modules.matcher import GeometricMaterialIdentifier
except ImportError as e:
    print(f"模块导入错误: {e}")
    print("请检查 modules 文件夹下的文件是否存在。")
    exit(1)

def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="SXM_Material_ID: STM图像自动化材料识别管道 (对称性驱动版)")
    parser.add_argument("-c", "--chemsys", type=str, required=True, help="化学系统，例如 'Fe-Se' 或 'Bi-Te'")
    
    # API Key
    parser.add_argument("-k", "--api_key", type=str, default=None, help="Materials Project API Key (覆盖 .env)")
    
    # 【核心容差参数设置】
    # 1. 绝对长度容差：默认放宽至 25%，包容压电陶瓷极大的标定漂移
    parser.add_argument("-tl", "--tol_len", type=float, default=0.25, 
                        help="晶格绝对长度容差比例 (默认: 0.25 即 25%)")
    
    # 2. 晶格比例容差 (新引入！)：默认 10%，严格校验四方/六方等形状对称性
    parser.add_argument("-tr", "--tol_ratio", type=float, default=0.10, 
                        help="晶格 a/b 比例容差 (默认: 0.10 即 10%)")
    
    # 3. 角度容差：默认 5.0 度，包容热漂移导致的剪切畸变
    parser.add_argument("-ta", "--tol_ang", type=float, default=5.0, 
                        help="晶格夹角绝对容差 (默认: 5.0度)")
    
    return parser.parse_args()

def select_folder():
    """调用系统窗口选择文件夹"""
    root = tk.Tk()
    root.withdraw()
    root.attributes('-topmost', True) 
    folder_path = filedialog.askdirectory(title="请选择包含 .sxm 文件的文件夹")
    return folder_path

def main():
    # 1. 初始化命令行参数与 API Key
    args = parse_arguments()
    final_api_key = args.api_key or os.getenv("MP_API_KEY")
    
    if not final_api_key:
        print("[错误] 未找到 Materials Project API Key！")
        print("请确保已在根目录创建 .env 文件并写入 MP_API_KEY=您的密钥。")
        return

    # 2. 弹出文件夹选择界面
    print("正在等待选择文件夹...")
    target_folder = select_folder()
    
    if not target_folder:
        print("[!] 未选择文件夹，程序已退出。")
        return

    # 3. 收集所有 .sxm 文件
    sxm_files = list(Path(target_folder).rglob("*.sxm"))
    if not sxm_files:
        print(f"[!] 在 {target_folder} 中未找到任何 .sxm 文件。")
        return

    print(f"\n========== SXM Material ID 批量处理 ==========")
    print(f"目标文件夹: {target_folder}")
    print(f"发现文件数: {len(sxm_files)} 个")
    print(f"先验化学系统: {args.chemsys}")
    print(f"容差策略: 绝对长度 ±{args.tol_len*100}%, 比例对称性 ±{args.tol_ratio*100}%, 角度 ±{args.tol_ang}°")
    print("==============================================\n")

    # 4. 设置结果输出路径
    result_dir = Path("results")
    result_dir.mkdir(parents=True, exist_ok=True)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_csv = result_dir / f"Batch_Report_{args.chemsys}_{timestamp}.csv"

    # 5. 初始化几何匹配器并预拉取数据库
    print("正在初始化 MP 数据库连接并拉取候选相 (包含所有高指数晶面)...")
    try:
        matcher = GeometricMaterialIdentifier(
            api_key=final_api_key, 
            tolerance_length=args.tol_len, 
            tolerance_ratio=args.tol_ratio,
            tolerance_angle=args.tol_ang
        )
        matcher._query_phase_library(chemsys=args.chemsys)
    except Exception as e:
        import traceback
        print(f"[错误] 无法连接到数据库或拉取数据失败，详细报错：")
        traceback.print_exc()
        return

    # 6. 打开 CSV 并开始批量处理流水线
    with open(report_csv, mode='w', newline='', encoding='utf-8-sig') as csv_file:
        # 扩展的表头，包含前三名匹配结果
        fieldnames = [
            'File Name', 'Exp_a (Å)', 'Exp_b (Å)', 'Exp_gamma (°)', 'Status', 
            'Rank 1 Match', 'Rank 1 Plane', 'Rank 1 Params', 'Rank 1 Score',
            'Rank 2 Match', 'Rank 2 Plane', 'Rank 2 Params', 'Rank 2 Score',
            'Rank 3 Match', 'Rank 3 Plane', 'Rank 3 Params', 'Rank 3 Score'
        ]
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writeheader()

        for idx, file_path in enumerate(sxm_files, 1):
            file_name = file_path.name
            print(f"\n[{idx}/{len(sxm_files)}] 正在分析: {file_name}")
            
            # 初始化行数据
            row_data = {key: 'N/A' for key in fieldnames}
            row_data['File Name'] = file_name
            row_data['Status'] = 'Failed'

            try:
                # 步骤 A: 读取原始数据
                z_data, physical_info = read_sxm(str(file_path))
                
                # 步骤 B: 实例化分析器并提取特征
                analyzer = STMAnalyzer(z_data, physical_info)
                lattice_results = analyzer.find_lattice_parameters()

                if lattice_results is None:
                    raise ValueError("未能在频域中提取到足够数量的 Bragg 峰。")

                # 单位换算 (nm -> Å)
                exp_a = lattice_results["a"] * 10.0
                exp_b = lattice_results["b"] * 10.0
                exp_gamma = lattice_results["angle"]
                
                # 实验角度的底层控制 (在 matcher 内部也会把钝角转锐角，这里统一打印方便查看)
                if exp_gamma > 90.0:
                    exp_gamma = 180.0 - exp_gamma
                
                row_data['Exp_a (Å)'] = f"{exp_a:.3f}"
                row_data['Exp_b (Å)'] = f"{exp_b:.3f}"
                row_data['Exp_gamma (°)'] = f"{exp_gamma:.1f}"
                print(f"  -> 提取晶格: a≈{exp_a:.3f}Å, b≈{exp_b:.3f}Å, γ≈{exp_gamma:.1f}°")

                # 步骤 C: 容差几何匹配 (寻找 Top 3)
                matched_results = matcher.match_experimental_data(
                    exp_a=exp_a, exp_b=exp_b, exp_gamma=exp_gamma, chemsys=args.chemsys
                )

                if matched_results:
                    row_data['Status'] = 'Success'
                    # 遍历前三名 (如果匹配结果少于3个，则按实际长度遍历)
                    for i in range(min(3, len(matched_results))):
                        res = matched_results[i]
                        plane_str = f"({''.join(map(str, res['Plane']))})"
                        
                        row_data[f'Rank {i+1} Match'] = res['Material']
                        row_data[f'Rank {i+1} Plane'] = plane_str
                        row_data[f'Rank {i+1} Params'] = res['Theoretical']
                        row_data[f'Rank {i+1} Score'] = f"{res['Error Score']:.4f}"
                        
                    print(f"  -> 最佳匹配: {matched_results[0]['Material']} | 晶面: {matched_results[0]['Plane']} | 得分: {matched_results[0]['Error Score']:.4f}")
                else:
                    row_data['Status'] = 'No Match'
                    print("  -> 警告: 当前容差下未找到任何匹配项。")

            except Exception as e:
                print(f"  -> [错误] 处理失败: {e}")
                row_data['Status'] = f"Error: {str(e)}"

            writer.writerow(row_data)

    print(f"\n========== 批量处理完成 ==========")
    print(f"已成功分析 {len(sxm_files)} 个文件。")
    print(f"详细的 Top-3 候选分析报表已保存至: {report_csv.absolute()}")

if __name__ == "__main__":
    main()